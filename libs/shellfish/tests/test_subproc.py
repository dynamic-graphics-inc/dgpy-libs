...
def test_pipe_stdout(tmp_path: Path) -> None:
    ...
    stdout = fs.read_string("stdout.txt")
...
def test_pipe_stderr(tmp_path: Path) -> None:
    ...
    stderr = fs.read_string("stderr.txt")
    assert stderr == proc.stderr

    proc >>= "stderr.txt"
    stdout = fs.read_string("stderr.txt")
...
def test_timeout_subprocess(tmp_path: Path) -> None:
    ...
    fs.write_string(script_2sec_filepath, script_2sec)
    fs.write_string(script_4sec_filepath, script_4sec)
